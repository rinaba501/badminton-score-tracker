...
- **Friends invite link (Roadmap Phase 7d, iOS-only)** — a `badminton://addfriend?id=<participantId>&name=<name>` deep link lets two people connect without sharing a club. Opening one on iPhone presents a confirmation sheet naming the inviter; nothing is sent until the user taps "Send Friend Request" (which requires CloudKit sync on — otherwise the sheet explains how to enable it, with no send button). Confirming publishes the sender's public `FriendProfile` (using the roster "Me" name until the 7e display-name prompt exists) and writes a `FriendRequest` to CloudKit's public database; self-invites and duplicate pending requests are rejected with a specific message. Link *generation* UI (ShareLink) and the Friends list screen arrive in Phase 7e — the Watch does not consume this link (its `badminton://` handler remains the `newmatch` complication deep link)
