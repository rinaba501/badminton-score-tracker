...
**Sync Backend (Roadmap Phase 9c-3, experimental)**: a "Sync Backend" section (both targets) lets this device opt its personal data (Settings + personal roster/match history) and, as of Phase 9d, clubs/challenges/reactions/club invites into Supabase instead of iCloud — Friends still syncs through iCloud regardless (Phase 9e, not started). On iPhone, "Sign in with Google" drives real Google Sign-In and relays the resulting session to a paired Watch; the Watch never signs in itself — it shows "sign in on your iPhone first" until a relayed session arrives, then offers its own explicit activation button, so a phone-side sign-in never silently switches the Watch's transport without a Watch-side confirmation. A "Switch Back to iCloud" button (behind a confirmation) reverts a device to iCloud sync at any time; personal data already written to Supabase isn't deleted, the device just stops writing to it. A Supabase-active club's member list, leave, and (owner-only) kick now work the same way they do on iCloud (Phase 9d-3) — kicking is a swipe action on the member row rather than the system share sheet's built-in participant management, since a joinable link has no such UI of its own. This is an early, per-device opt-in toward an eventual full backend migration (see `docs/supabase-migration-plan.md`) — not yet feature-complete (Friends still CloudKit-only) and not yet verified against a real two-device/real-account pass.
