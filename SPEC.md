...
- **Sync:** local-only by default; opt into Supabase/Postgres via the Sync Backend section in Settings (see "Sync Backend" below) to sync across devices — one row per match/player/club plus a `settings` row carrying every scalar setting (`myName`, `localPlayerId`, `pointsToWin`, `gamesInMatch`, `courtTheme`, `announceScore`, `enableSounds`, `enableCrownScoring`, `timeModeEnabled`, `timeLimitMinutes`). Correctness still needs a real two-device test (not CI-provable)
...
- **Match History and Stats are on iPhone.** A dashboard-style home screen (quick-stats strip: matches / win rate / players — each cell navigates to History/Stats/Players respectively — plus a prominent New Match button with a smaller "Add Match" (manual entry, #278) button directly beneath it, and a "last match" card below the menu once at least one match exists) links to a full Match History (date-range / match-type / sort / multi-player filters, avatar'd rows, swipe-to-delete, clear-all) and a Stats screen (win-rate ring, stat cards, head-to-head with avatars) — the same information and derivations as the Watch, restyled for the phone. Deleting a match on iPhone removes it everywhere via iCloud.
- **My Profile is its own page on iPhone**, reached from the home menu: edit your own name/avatar, separate from the shared roster of other players. Renaming yourself here updates your name in every past match. iOS only — watchOS keeps "Me" as a distinguished row inside Settings/Roster since the small screen doesn't warrant another top-level destination.
...
- **Cross-device sync is live once signed in:** the iPhone reads (and can edit/delete) the same match history, roster, clubs, and identity/match-format settings the Watch records, once both devices are signed into the same Supabase account (see "Sync Backend" below — iPhone drives the actual sign-in, relays the session to a paired Watch). The Watch stays the richest scoring device.
...
- **Correct Score (#279)** — the In-Match Settings sheet's Correct Score section has a Stepper per side that jumps the current game's score directly, for a scorekeeping mistake further back than Undo's one-point-at-a-time reach. Each tap is independently undo-able (same as a real point). Hidden once the game/match is decided; a correction that would itself end the game (or is out of the valid 0–pointCap range) is silently rejected. Doesn't touch doubles serve/box-partner state, since that's derived from *how* a score was reached, not the score value alone — the scorekeeper can still walk it back into agreement with a few real taps if a doubles pair's court positions also need fixing
...
- **In-Match Settings** — a gear button in the toolbar opens a compact sheet with the settings that already apply live during a match: Sound Effects, Score Announcement, Court Theme, and (Watch-only) Digital Crown — plus the Correct Score section (#279) above. Match Format, Match Timer, and Court Change Reminders stay full-Settings-only — changing them mid-match would affect the in-progress score/timer state and needs its own design pass
- **Score Screen Style (iOS only)** — 8 selectable visual styles for the live scoring screen: Depth (gradient/glass tiles, theme-driven accents — closest to the original layout; the serving tile is marked in solid white — 3pt border + soft glow + a white court-label chip — rather than the theme tint, which doesn't read against the tinted glass), Split (full-bleed diagonal split-screen; the currently-serving side's field takes the Court Theme color plus a pulsing red edge-light and serve dot, the other side stays neutral; scores carry a deep layered drop shadow and spring-animate on change), Minimal (flat two-row layout, hairline divider, restrained theme-tinted accent, subtle theme-tinted background wash), Blackbird (broadcast-style scorebar on a black field; serve is a shape marker, not a color — dimmed rather than hidden on the non-serving side — with a thin theme-colored edge-light; the screen splits into two equal halves at the scorebar's divider and tapping anywhere in a half scores that side, flashing the half to confirm), LED Board (skeuomorphic retro LED gym scoreboard — glowing digits over a dim ghost digit, lit "SERVE" lamp per side, games-won as lamp dots), Birds-Eye (top-down court-diagram background; the serve marker sits in the correct service court per real serving rules, games-won as a fixed-slot row of glowing dots — one dot per game needed to win the match, same legible-tally treatment as LED Board's lamp dots), Tug (kinetic style — the divider between sides drags toward whoever's ahead and numerals spring-animate on each point; respects Reduce Motion), Scoreboard (the manual courtside flip-card scoreboard — cards on binder rings with a hinge seam, a small games-won card beside each score card, name plate under each stack, theme-colored plates and base rail; each point swings the card off its top hinge, skipped under Reduce Motion — an undo swings it the opposite rotational direction instead of replaying the same forward flip, with a matching percussive "clack" cue (a low thud + high tick sine burst, gated behind the Sound Effects setting like every other score cue) on every flip in either direction; serve is a lamp on the serving side's plate. **The app's one landscape style**: entering a match with Scoreboard active rotates the game screen to landscape and exiting restores portrait — every other screen stays portrait-only. Home/"me" team sits left, visitor right). Free for everyone, no Pro gating. Birds-Eye is the default for a fresh install. The Watch's Game Screen is unaffected — it has its own layout and no style picker
...
- **Select mode (bulk delete, #263)** — a "Select" entry (Watch: `⋯` menu next to Clear All; iOS: `⋯` menu) enters a select mode where tapping a row toggles it instead of the normal swipe/share action; a "Select All"/"Deselect All" control scopes to the currently filtered/visible matches, not the full unfiltered history; a "Delete (n)" action removes every selected match after one confirmation. Selection is cleared on exiting select mode or after a delete
...
- **Friend opponent links to their profile** — when a match's opponent was picked from Friends (`opponentParticipantId` set) and is still an accepted friend, tapping their name opens the same friend-profile screen Friend Activity uses (identity/stats/history, whichever they've shared); unfriending removes the link, not just the avatar. Off while in Select mode, so tapping a row toggles selection instead
- **Add a past match manually (#278, iOS-only)** — a "+" button (on History's toolbar, and next to the New Match button on the top screen, since this is expected to be a frequently used action) opens a form that produces a `MatchRecord` without playing it live: game mode (Singles/Doubles), near/far players and partners (same roster/friends/guest picker as Pre-Match, any slot in any order), one score row per game, a date (defaults to now, can't be set in the future), and the same Club/Official Match picker Pre-Match offers. A Games Stepper (with both + and − controls) sets how many score rows are shown, defaulting to however many the device's current Match Format setting needs to decide a match (Best of 3 → starts at 2 rows) — the count isn't capped to "Best of N" odd totals, since a real result can legitimately have an even game count (a straight-sets sweep) or an odd one (a match that went to a decider); a non-blocking warning appears if the entered count comes out even, as a nudge to double-check nothing was missed, not an error. Swipe to remove a specific row (down to a minimum of 1) independent of the Stepper. Games won and the winner are derived from the entered scores — Save is disabled until every game has a non-tied score and the match itself doesn't end in a tie. iOS-only: the date field needs a `DatePicker`, and this app has no `DatePicker`-editing precedent on Watch (see Club Seasons below) — manual score entry alone would have been Watch-feasible, but the date field wasn't
...
- **Score Screen Style** — Depth / Split / Minimal / Blackbird / LED Board / Birds-Eye / Tug / Scoreboard (see Game Screen above; Scoreboard is landscape-only). **iOS-only** — no equivalent on Watch. Default is Birds-Eye
...
**Danger Zone — Erase All My Data (#264)**: a Settings entry (both targets) that permanently deletes every local AND cloud-synced record the account owns — roster, match history, clubs, all scalar settings, and the Friends graph (profile + friend requests + identity/stats snapshot rows) — leaving the app in a fresh-install state (`myName`/the first-launch name prompt reset too). Gated behind a type-to-confirm text field (the user must type the fixed keyword `DELETE`) since this is the one action whose blast radius can reach other people's shared data, not just the user's own. Any club the user owns is listed by name and deleted outright as part of the erase — real club-ownership *transfer* isn't supported (both backends tie a club's ownership permanently to its creating account); a club owner who wants to keep a club alive for its members must transfer it manually before erasing (no in-app transfer flow exists yet — see Open Issues).

**Sync Backend (Roadmap Phase 9, complete)**: a device is local-only by default — nothing leaves it — until it explicitly signs in. A "Sync Backend" section (both targets) does that sign-in and, once linked, syncs everything through Supabase: personal data (Settings + personal roster/match history), clubs/challenges/reactions/club invites, and the Friends graph (requests, identity/stats sharing, history sharing), all gated by the same per-field opt-in toggles as before. On iPhone, "Sign in with Google" drives real Google Sign-In and relays the resulting session to a paired Watch; the Watch never signs in itself — it shows "sign in on your iPhone first" until a relayed session arrives, then offers its own explicit activation button, so a phone-side sign-in never silently switches the Watch's transport without a Watch-side confirmation. A "Stop Syncing" button (behind a confirmation) reverts a device to local-only at any time; data already written to Supabase isn't deleted, the device just stops writing to it — there is no other backend to fall back to (CloudKit has been fully retired, see `docs/supabase-migration-plan.md`). A club's member list, leave, and (owner-only) kick work the same way for every device. This is per-device — signing in on one device doesn't sign in another — and not yet verified against a real two-device/real-account pass.
...
- **Select mode (bulk delete, #263)** — a "Select" button in the Players list (Watch: inline in the Players section of Settings; iOS: a toolbar button in the dedicated Roster screen) enters a select mode where tapping a player toggles it instead of opening the editor; "Select All"/"Deselect All" scopes to the currently sorted list of players; a "Delete (n)" action removes every selected player from the roster (their name still appears in past matches — only the roster entry is removed) after one confirmation
- **Clubs (Roadmap Phase 5d)** — an opt-in local grouping of players/history, entered from Settings (Watch) or the main menu (iOS): create/rename a club, view its member list (shows "You" always; other members appear once the club has been shared with them — see Club invites below), and view/manage its roster by tagging a player with the club from the player editor's "Club" picker (default "Personal"). Deleting a club you own, or leaving one shared to you, never deletes players or match history — it only clears their club tag back to Personal
- **Club invites (Roadmap Phase 5e/9d-2)** — a club's owner can tap "Invite…" in its member list, shown when this device is signed into the Sync Backend (Settings): creates a joinable link and shares it via the standard `ShareLink` sheet, on **both** iPhone and Watch; opening the link (iOS only, same split as Friends' invite link — sendable from either target, consumable on iPhone) shows a "Join <club>?" confirmation before actually joining
...
- **Club standings (Roadmap Phase 5 backlog, #159)** — `ClubDetailView` (both targets) shows a per-club leaderboard (name, wins-losses) sorted by win rate then wins; your own entry (your row in the Members section, and whichever side of an Activity feed row is you) all carry a small "you" badge instead of a separate label, so your name reads like everyone else's. Every Members/Standings/Activity/Friends row also shows an avatar circle next to the name — your own and any club opponent with a saved roster entry show their actual customized avatar, and an accepted friend's row (in Friends and Friend Activity — see below) shows their real avatar too, once it's synced; everyone else falls back to plain initials on gray (remote-only members you're not friends with aren't local roster entries with a saved color/icon). Standings can be turned off entirely — see the Track Standings toggle below
- **Match confirmation (Roadmap Phase 5 backlog, #160)** — a club owner can require matches to be confirmed before they count toward standings (default off, invisible for personal matches). Pending matches show in a Pending Confirmation section on `ClubDetailView` with Confirm/Decline actions; declining returns the match to Personal rather than deleting it. Practice matches (see below) never enter this queue, since a match that never counts toward standings has nothing to gate
- **Tag a new match with a club (Roadmap Phase 5 backlog, #169)** — `PreMatchView` (both targets) offers the same "Club" picker (default "Personal") on its near-side player step, hidden entirely for a solo user with no clubs; the selection is threaded into the saved `MatchRecord.clubId`, making club standings (#159) and match confirmation (#160) reachable through normal play. When the selected club tracks standings, an "Official Match" toggle appears alongside the club picker (default on); turning it off tags the match as practice via `MatchRecord.isOfficial`
- **Club seasons (Roadmap Phase 5 backlog, #163)** — a club owner (iOS-only editing, `ClubDetailView`) can set an optional Season window (a start date, plus an independently optional end date). When set, the Standings section only counts matches recorded within that window (inclusive; no end date means open-ended); the Activity Feed is unaffected — it stays a full chronological log regardless of season. No season set (the default) leaves standings exactly as before. The Standings section shows a read-only "Season: `<start>` – `<end/present>`" label whenever one is active, visible to owner and non-owner alike; Watch shows the same read-only label but has no editing controls, matching the existing DatePicker-editing-is-iOS-only precedent (birthday, introduction). The Season section itself is hidden whenever Track Standings is off, since Season only exists to feed Standings
- **Track Standings toggle** — a club owner can turn Standings tracking off for the club entirely (default on, unchanged behavior), on both targets. When off, both the Standings section and the Season section disappear from `ClubDetailView`, leaving the club as pure score-tracking/history with no leaderboard or competitive pressure
- **Official vs. practice matches** — when recording a match into a club that tracks standings, `PreMatchView` offers an "Official Match" toggle (default on). Turning it off marks the match Practice: it still appears in the club's Activity Feed (tagged "Practice"), but is filtered out of Standings and never enters the Pending Confirmation queue, regardless of the club's match-confirmation setting
- **Reactions & comments on a club match (Roadmap Phase 5 backlog, #164)** — each club activity-feed entry supports emoji reactions (👍 🔥 🏸 😮; tap to toggle, one per member per emoji) and one-line comments (≤200 chars). Every activity row shows the exact per-game score (e.g. "21-18, 15-21, 21-19"), not just the games-won tally — same format History's per-game line uses. Watch: the feed row shows a count summary and pushes a detail screen with emoji toggling and a read-only comment list; iOS: emoji chips sit inline on the row, and a comment sheet lets comments be composed (real keyboard — the Watch never authors comments, only reads them). Stored as `ReactionRecord`s, club-scoped only like challenges: authoring needs to be signed into the Sync Backend plus a resolved participant identity, otherwise the controls are disabled. Reactions on a deleted match/club become invisible orphans (reads join on `clubId`+`matchId`) and are never purged or resurrected. A new reaction from a clubmate also lights the club's unread-activity dot
- **Friends invite link (Roadmap Phase 7d, iOS-only)** — a `badminton://addfriend?id=<participantId>&name=<name>` deep link lets two people connect without sharing a club. Opening one on iPhone presents a confirmation sheet naming the inviter; nothing is sent until the user taps "Send Friend Request" (which requires being signed into the Sync Backend — otherwise the sheet explains how to enable it, with no send button). Confirming publishes the sender's public profile and writes a `FriendRequest` row; self-invites and duplicate pending requests are rejected with a specific message. The Watch does not consume this link (its `badminton://` handler remains the `newmatch` complication deep link)
- **Friends screen (Roadmap Phase 7e, both targets)** — a Friends entry point (next to Clubs, iOS shows a numeric badge for unread incoming requests) opens a list of incoming pending requests (Accept/Decline), outgoing pending requests (Cancel), and the current friends list. An "Add Friend" row shares the user's own invite link via the system share sheet (`ShareLink`) — works on both iPhone and Watch, since a friend invite is just a URL with no keyboard involved. A second "Enter a Code" row (Roadmap Phase 7f) is a fallback for when the link doesn't transfer cleanly: paste either the full invite link or just the bare code, and the app looks up that player's profile before sending the request, showing a clear error if no such player exists. Incoming requests arrive live via a Realtime subscription while the app is open, plus a poll on appear/pull-to-refresh as a backstop. Friends always shows/sends your existing scoring name ("Me" in Settings/Roster) — there's no separate name to set up first. Everything here requires being signed into Supabase (Sync Backend section in Settings)
...
- **Friend badges in a club's member list** — every club member (including you) gets a small badge if they're already one of your accepted Friends, a simple id cross-reference between Club membership and the Friends graph, no separate invite/permission needed
- **Share your profile with friends, field by field** — a "Sharing Settings" row at the bottom of Friends (both targets, grouped with "Friend Activity" under a trailing "More" section since neither is a day-to-day action) opens a dedicated screen with five independent toggles, each on/off: gender, birthday, "about me" bio, stats (win rate/games played/head-to-head), and match history — so you can, say, share your win rate with friends without exposing every match. All default off; your name and avatar are the two things that are never toggle-gated (#272 — an avatar isn't sensitive data, so it's always visible to accepted friends just like your name is, the moment they accept). Gender/birthday/bio are edited on iOS only (ProfileView — ID/DatePicker/a 200-char bio field), same iOS-only-authoring precedent as club match comments; identity/stats are precomputed snapshots (not raw match records), so turning on Stats never exposes your actual games unless History is also on. Turning any toggle on makes that field visible to every accepted friend (row-level access, not a joinable link, unlike Club invites) and stays in sync as the friend graph changes; turning off the *last* remaining toggle removes visibility for everyone (avatar/name stay visible regardless). A friend's shared data is never merged into your own — it's shown read-only in a "Friend Activity" list (tap a friend's name to see whichever of their identity/stats/history they've actually shared, reusing the same match-row formatting as your own History screen for the history part). On iOS, the gender/birthday/bio toggles are also reachable inline: a small share icon sits right next to each field in Profile, so the decision can be made the moment that field is filled in, not just from the dedicated Sharing Settings screen. Since the icon has no visible text, tapping it briefly shows a "Shared with friends"/"No longer shared with friends" toast so the effect is unambiguous. History and Stats get the same inline treatment, since match history/stats have no natural field to sit next to in Profile: iOS History shows a full "Share My History with Friends" switch (with its explanatory footer) at the top of the list whenever you're viewing your personal matches, and iOS Stats shows the same icon-only toggle + toast as Profile's fields whenever you're viewing your own personal stats
...
- Sync is per-record: a new match recorded on either device upserts its own row; deleting a match or clearing history enqueues real per-record deletes (no blob merge / overwrite heuristic)
...
| # | Feature | PR |
|---|---|---|
| 279 | Correct Score: a Stepper per side in In-Match Settings jumps the current game's score directly, for a correction bigger than Undo's one-point reach — new `BadmintonMatch.setScore`/`canSetScore` reject out-of-range values and anything that would itself end the game, and each Stepper tap is its own undo-able snapshot | #328 |
