...
- **Club invites (Roadmap Phase 5e/9d-2)** — a club's owner can tap "Invite…" in its member list, shown when this device's Sync Backend is set to Supabase (Settings): creates a joinable link and shares it via the standard `ShareLink` sheet, on **both** iPhone and Watch; opening the link (iOS only, same split as Friends' invite link — sendable from either target, consumable on iPhone) shows a "Join <club>?" confirmation before actually joining. The original iOS-only `CKShare`-based invite (`UICloudSharingController`) predates the Supabase migration and is not reachable on a device that hasn't switched Sync Backend to Supabase (Roadmap Phase 9f, in progress) — a fresh install has no default cloud sync at all until it signs in
