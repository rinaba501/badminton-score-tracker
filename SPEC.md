...
- **Club standings (Roadmap Phase 5 backlog, #159)** — `ClubDetailView` (both targets) shows a per-club leaderboard (name, wins-losses) sorted by win rate then wins; your own entry (and your row in the Members section above it) carries a small "you" badge instead of a separate label, so your name reads like everyone else's. Every Members/Standings/Friends row also shows an avatar circle next to the name (initials on a gray background, since friends and club members aren't local roster entries with a saved color/icon)
