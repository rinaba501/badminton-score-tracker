...
- **Club invites (Roadmap Phase 5e, iOS-only)** — a club's owner can tap "Invite…" in its member list (shown only when CloudKit sync is on) to send a `CKShare` invitation via the system share sheet (`UICloudSharingController`); no watchOS equivalent exists. On a device with the experimental Sync Backend switched to Supabase (Roadmap Phase 9d-2), the same Invite button instead creates a joinable link and shares it via the standard `ShareLink` sheet — this path works on **both** iPhone and Watch, since a link needs no CKShare; opening the link (iOS only, same split as Friends' invite link — sendable from either target, consumable on iPhone) shows a "Join <club>?" confirmation before actually joining
