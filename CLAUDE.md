...
- `BadmintonMatch` — pure scoring engine; no UI, no timers. Tracks scores, games won, serve side, win conditions. Has no concept of individual player identity at all (names live entirely in the app layer) — for doubles it only tracks `myRightBoxPartnerIndex`/`opponentRightBoxPartnerIndex` (which of a team's two partners, 0 or 1, currently occupies the right-hand service court) plus `currentPartnerIndex(for:)` to derive who should be serving/receiving next. This is meaningless in singles (stays 0) and doesn't require a `gameMode` field on the engine at all — the app decides whether to interpret/display it. The one rule this encodes: a team's partners swap right/left occupancy only when that team wins a rally while already serving; a side-out never moves either team's partners
- `MatchRecord` — persisted match result; stores player names + optional `UUID` player IDs for name-change tracking. `myPartnerName`/`opponentPartnerName`/`myPartnerPlayerId`/`opponentPartnerPlayerId` are optional doubles fields — a record is doubles precisely when either partner name is non-nil (no separate `isDoubles`/`gameMode` flag); old singles JSON decodes these as `nil` with no migration needed, since Swift's synthesized `Codable` treats a missing key for an `Optional` property as `nil`
...
| `matchMyPartnerName` | `String` | Near-side team's second player, for doubles (reserved; not yet read by any view) |
| `matchOpponentPartnerName` | `String` | Far-side team's second player, for doubles (reserved; not yet read by any view) |
