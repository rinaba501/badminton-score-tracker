...
badminton score tracker/       — iOS companion app (#41, in progress). Same target that used to be the watch-only stub container — now a real iOS app (NavigationStack root, iPhone-only, iOS 17+) that still embeds the Watch App. Has its own Info.plist, Assets.xcassets, .entitlements, and 6 *.lproj tables (iOS-only chrome under ios.*; History/Stats screens reuse the Watch's own string keys verbatim so translations copy across); the Watch App's Info.plist now declares it as companion (WKCompanionAppBundleIdentifier) instead of WKWatchOnly
  ContentView.swift          — iOS root: NavigationStack + List menu linking to History, Stats, and Friends (a numeric badge shows the incoming pending-request count, from a cached myParticipantId lookup mirroring AppStore.friends). Also owns onOpenURL for the `badminton://addfriend` invite link (scheme registered in this target's Info.plist, Roadmap 7d), presenting FriendInviteView as a sheet, and the same first-launch skippable name prompt as the Watch's
  ...
  FriendsView.swift          — Friends v1 (Phase 7e): incoming/outgoing pending FriendRequests (accept/decline/cancel via CloudKitSyncManager.respondToFriendRequest), the friends list (AppStore.friends), and an add-friend ShareLink over FriendInviteLink.url. Same avatarColor(for:)/avatarIcon(for:)-backed AvatarView treatment as the Watch's on every name row. iOS restyle of the Watch's — ShareLink works on watchOS too (a friend invite is just a URL), so unlike ClubDetailView's CloudSharingView invite (UIKit-only), this view is symmetric across targets. Same first-launch-skip backstop name prompt as the Watch's (refresh()/linkAccount()/sendCode() all pause behind it while myName is still the default placeholder). Phase 7f adds a second "Enter a Code" row: pastes either a full invite link (parsed via FriendInviteLink.parse) or a bare participantId, validated via CloudKitSyncManager.fetchProfile before sending. Phase 7g adds an "Account" section (link/unlink toggle, SettingsSnapshot.accountLinked) — byte-identical to the Watch's addition
  ...
  GameView.swift             — iOS live scoring: NavigationStack shell (toolbar, discard alert, ticker, point-banner/match-over overlays) shared across all 3 GameScreenStyles; ScoreSideData/GameHeaderData are the shared per-side/per-header data structs each style renders from. Owns the Depth style (default — gradient/glass ScoreView tiles, two big tap tiles, games header, timer, banners). Tap-only — NO Digital Crown, NO HealthKit workout. Restyle of the Watch's
  GameScreenStyle.swift      — iOS-only: the 3-case GameScreenStyle enum (Depth/Split/Minimal) behind AppStorageKeys.gameScreenStyle, free for everyone (no isPremium/paywall gating unlike CourtTheme). Also hosts Color.blended(toward:by:), a manual RGB blend used to derive light theme-tinted accents — Color.mix(with:by:) needs iOS 18, this app targets 17
