...
BadmintonCore/                 — local Swift package; platform-free core (no SwiftUI/WatchKit imports, ever)
  Sources/BadmintonCore/
    ...
    SettingsSnapshot.swift    — every scalar setting synced as one personal-zone CloudKit Settings record (myName/localPlayerId/pointsToWin/gamesInMatch/courtTheme/announceScore/enableSounds/enableCrownScoring/timeModeEnabled/timeLimitMinutes/clubLastViewedActivity/accountLinked). Custom Codable (decodeIfPresent-with-default) so fields added after the first records shipped still decode — same self-migration MatchRecord.winner uses. clubLastViewedActivity is merged per-club (max date) on apply, not overwritten (AppStore.applyRemoteSettings). accountLinked (blind-overwritten like the other plain Bools) is the explicit "link this device to one CloudKit account" opt-in — see FriendsView's Account section. myName is the only identity string in the app (no separate Friends display name — see ClubDetailView/FriendsView)
