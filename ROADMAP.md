...
| 6 — iOS companion app | [#41](https://github.com/rinaba501/badminton-score-tracker/issues/41) | In progress — see [docs/ios-companion-app-plan.md](docs/ios-companion-app-plan.md) for the full PR-by-PR plan and status. PR1 (target restructure + shell + CI) and PR2 (iCloud KV sync layer — iPhone shares the Watch's bucket, two-device test pending) done; remaining: History/Stats, Roster, Share (#13), then live scoring. Watch app is no longer WKWatchOnly as of PR1 — archive an earlier commit for a watch-only App Store submission |
