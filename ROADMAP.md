...
| 6 — iOS companion app | [#41](https://github.com/rinaba501/badminton-score-tracker/issues/41) | In progress — see [docs/ios-companion-app-plan.md](docs/ios-companion-app-plan.md) for the full PR-by-PR plan and status. PR1 (target restructure + shell + CI), PR2 (iCloud KV sync layer — iPhone shares the Watch's bucket, two-device test pending), PR3 (History + Stats views), PR4 (Roster management), and PR5 (Share card, closes #13) done; remaining: live scoring on iPhone. Watch app is no longer WKWatchOnly as of PR1 — archive an earlier commit for a watch-only App Store submission |
