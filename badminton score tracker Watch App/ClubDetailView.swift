...
//  joined against `profiles`), and always shows a self row (myRow, badged
//  rather than labeled "You" — see youBadge) as a fallback first row so
//  viewing a club never depends on being signed in — see the local-first
//  invariant in ROADMAP.md. Gets an owner-only swipe-to-kick action too.
...
//  diffing that already routes owned vs. shared deletion correctly.
//
...
struct ClubDetailView: View {
            ...
            if let club {
                Section(header: Text("clubs.name")) {
                    ...
                    if isOwned {
                        ...
                        .font(.caption)
                        // Needs to be signed into Supabase to create an
                        // invite.
    ...
    /// Creates a `club_invites` row and builds a `ClubInviteLink` URL. Only
    /// reachable when `supabaseAccountLinked`.
...
/// A `ShareLink` over the freshly-created invite URL.
